...
class TestProverbCSVCleaner(unittest.TestCase):
    ...
    def test_real_data_file(self):
        ...
        input_file = DATA_DIR / "Proziceri.csv"
        if not input_file.exists():
            self.skipTest(f"Proziceri.csv not found at {input_file}")
        ...
        # Should parse entries from the file
        # (Note: parser may skip entries, so just verify it parsed something)
...
class TestCleanedCSVFile(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test fixtures"""
        self.csv_file = DATA_DIR / "Proziceri.csv"
        if not self.csv_file.exists():
            self.skipTest(f"Proziceri.csv not found at {self.csv_file}")
    ...
    def test_separator_distribution(self):
        ...
        # Should have comma separators (quoted commas are unquoted when read)
        self.assertIn(',', separator_counts)
